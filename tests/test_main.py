...
from main import (
    APRS_DATA_TABLE_ID,
    EARTH_RADIUS_KM,
    GPX_SYMBOL_LAST_SEEN,
    GPX_SYMBOL_PREDICTED_LANDING,
    GPX_SYMBOL_RADIOSONDY_LANDING,
    Coordinates,
    SondeData,
    SondeProcessor,
)
...
class TestSondeData(unittest.TestCase):
    ...
    def test_sonde_data_creation(self):
        ...
        sonde = SondeData(
            ...
            last_seen_time=datetime(2024, 1, 1, 12, 0, 0, tzinfo=UTC),
            course=319.0,
            altitude=544.0,
            speed_mps=5.56,
            climb_rate=-7.0,
...
class TestCoordsParsing(unittest.TestCase):
    ...
    def test_parse_coords_with_timestamp(self):
        ...
        processor = SondeProcessor(
            "https://radiosondy.info/sonde_archive.php?sondenumber=D20040532",
            coords_str,
        )
    ...
    def test_parse_coords_negative_values(self):
        ...
        processor = SondeProcessor(
            "https://radiosondy.info/sonde.php?sondenumber=X3432089", coords_str
        )
        self.assertIsNotNone(processor.radiosondy_coords)
...
class TestLandingPointCalculation(unittest.TestCase):
    ...
    def test_calculate_landing_point_basic(self):
        ...
        processor = SondeProcessor(
            "https://radiosondy.info/sonde_archive.php?sondenumber=D20040532", None
        )
        coords = Coordinates(lat=49.915, lon=9.5154)
...
class TestGPXFileGeneration(unittest.TestCase):
    ...
    def test_create_gpx_file(self):
        ...
        sonde_data = SondeData(
            last_seen_coords=Coordinates(lat=49.915, lon=9.5154),
            last_seen_time=datetime(2024, 1, 1, 11, 52, 0, tzinfo=UTC),
            course=319.0,
            altitude=544.0,
            speed_mps=5.56,
            climb_rate=7.0,
        ...
        with tempfile.TemporaryDirectory() as tmpdir, patch.object(
            SondeProcessor, "create_gpx_file", return_value=f"{tmpdir}/test.gpx"
        ):
...
class TestEarthRadiusCalculation(unittest.TestCase):
    ...
    def test_landing_distance_calculation(self):
        ...
        landing_point, _ = processor.calculate_landing_point(
            coords=coords,
            altitude=1000.0,
            speed=10.0,
            course=0.0,
